...
final class AiController
{
    ...
    private function buildSystemPrompt(string $date, int $workStart, string $workEnd, string $timezone): string
    {
        ...
        return implode("\n", [
            'Je bent een persoonlijke productiviteitsassistent voor een gebruiker\'s Life OS.',
            'Jouw taak is een realistisch dagschema te maken dat:',
            '- Vaste kalenderafspraken (vergaderingen, afspraken) respecteert',
            '- Reistijd herkent: als een kalender-evenementtitel woorden bevat zoals',
            '  "trein", "train", "intercity", "NS", "Thalys", "Eurostar", "vlucht", "flight",',
            '  "bus", "metro", "ferry", "reisadvies" — behandel het als een reisblok waar',
            '  de gebruiker licht leeswerk of eenvoudige taken kan doen (geen diep werk)',
            '- Taaklengtes intelligent inschat: gebruik estimated_mins indien aanwezig;',
            '  schat anders op basis van de taaknaam en omschrijving (typische ranges:',
            '  snelle admin 15min, e-mail/review 20-30min, gefocust werk 45-90min, diep',
            '  werk 90-120min)',
            '- Oppervlakkige taken groepeert waar dat zinvol is',
            '- Korte pauzes (10-15 min) invoegt na elke 90 minuten gefocust werk',
            '- Pauzes mogen nooit overlappen met taken, kalenderblokken of reisblokken — plaats ze alleen in echte gaten',
            '- Geen werk inplant buiten ' . $startLabel . '–' . $workEnd,
            '- ALLEEN geldige JSON teruggeeft — geen markdown, geen uitleg, alleen JSON',
            '',
            'Datum van vandaag: ' . $date,
            'Tijdzone: ' . $timezone,
            'Huidige lokale tijd: ' . $nowLabel,
            'Werkuren gebruiker: ' . $startLabel . ' – ' . $workEnd,
        ]);
    }
    ...
    private function buildUserPrompt(array $tasks, array $events, int $workStart, string $workEnd, string $timezone): string
    {
        ...
        if ($eventLines === []) {
            $eventLines[] = '- Geen kalenderafspraken vandaag';
        }

        return implode("\n", [
            'Dit zijn mijn openstaande taken (prioriteit 4=urgent, 1=laag):',
            implode("\n", $taskLines === [] ? ['- Geen openstaande taken'] : $taskLines),
            '',
            'Kalenderafspraken van vandaag:',
            implode("\n", $eventLines),
            '',
            'Reageer met exact deze JSON-structuur:',
            '{',
            '  "schedule": [',
            '    {',
            '      "start_time": "09:00",',
            '      "end_time": "10:30",',
            '      "task_id": 12,',
            '      "task_title": "Projectvoorstel schrijven",',
            '      "notes": "Diep werkblok — notificaties uitzetten",',
            '      "is_break": false,',
            '      "is_transit": false,',
            '      "is_calendar": false,',
            '      "estimated_mins": 90',
            '    }',
            '  ],',
            '  "reflection": "Je hebt 3 hoge prioriteit taken en een treinreis van 2 uur om 14:00, ideaal voor de lichtere reviewtaken.",',
            '  "suggested_tasks": [',
            '    {',
            '      "task_id": 5,',
            '      "action": "defer",',
            '      "reason": "Lage prioriteit en onvoldoende tijd vandaag vanwege de trein om 14:00"',
            '    }',
            '  ],',
            '  "score": 72',
            '}',
            '',
            'Regels:',
            '- Je MOET voor ELKE kalenderafspraak hierboven een scheduleblok opnemen met exact dezelfde lokale start_time en end_time, task_id null, is_break false, is_calendar true, task_title = naam van de afspraak',
            '- Dupliceer kalenderafspraken niet als aparte taakblokken',
            '- task_id moet een echt id uit de takenlijst zijn, of null voor pauzes/kalender/adminblokken',
            '- is_transit: true als dit blok een kalenderafspraak is die eruitziet als reizen',
            '- Plan geen taken tijdens kalenderafspraken tenzij is_transit true is (reisblok = lichte taken mogelijk)',
            '- Pauzeblokken: task_id null, is_break true — nooit overlappend met andere blokken (taken, kalender, reisblok)',
            '- Als je plant voor vandaag (' . $nowDate . '), plan geen enkel blok dat begint vóór ' . $nowTime . ' lokale tijd',
            '- Als er onvoldoende tijd overblijft, plan die taak dan niet in het schema; zet hem in suggested_tasks met action "defer" of "reschedule"',
            '- Dek de volledige werkdag af van ' . sprintf('%02d:00', $workStart) . ' tot ' . $workEnd . ' (' . $timezone . ')',
            '- Als een taak geen estimated_mins heeft, kies een realistische duur en stel estimated_mins in je antwoord in',
            '- Reageer uitsluitend met geldige JSON',
        ]);
    }
    ...
    private function buildCalendarBlocksFromEvents(array $events, string $planDate, \DateTimeZone $tz, ?\DateTimeImmutable $clipNow): array
    {
        ...
        foreach ($events as $ev) {
            ...
            $out[] = [
                ...
                'task_id' => null,
                'task_title' => $title,
                'notes' => $loc !== '' ? '@ ' . $loc : 'Kalenderafspraak',
                'is_break' => false,
